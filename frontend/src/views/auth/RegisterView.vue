    ...
    <div class="w-full max-w-md">
      <div class="glass-card p-10">
        <div class="text-center mb-10">
          ...
          <h1 class="text-3xl font-display font-black text-primary-900 tracking-tight mb-2">Create Account</h1>
          <p class="text-primary-500 font-medium">Join the next generation of property analysis</p>
        </div>
        ...
        <form @submit.prevent="handleRegister" class="space-y-6">
          ...
          <div>
            <label class="block text-xs font-black text-primary-400 uppercase tracking-widest mb-3">Full Name</label>
            <div class="relative">
              <UserIcon class="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-primary-300" />
              <input 
                v-model="fullName" 
                type="text" 
                required 
        ...
        <div class="mt-10 text-center">
          <p class="text-primary-500 font-medium">
            ...
            <RouterLink to="/login" class="text-brand-600 font-black hover:underline">Sign in here</RouterLink>
          </p>
        </div>
      </div>
    </div>
...
const handleRegister = async () => {
  ...
  try {
    ...
    error.value = err.response?.data?.detail || 'Registration failed. Email might be taken.'
  } finally {
    loading.value = false
  }
}
